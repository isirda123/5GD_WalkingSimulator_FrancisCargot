...
namespace StylizedWaterShader
{
    public class StylizedWaterCore : Editor
    {
        ...
        public const string INSTALLED_VERSION = "2.0.9";
        ...
        private static bool WindowAppeared
        {
            get { return SessionState.GetBool("SWS_POPPEDUP", false); }
            set { SessionState.SetBool("SWS_POPPEDUP", value); }
        }
        ...
        {
            //Get script path
            string[] scriptGUID = AssetDatabase.FindAssets("StylizedWaterCore t:script");
            string scriptFilePath = AssetDatabase.GUIDToAssetPath(scriptGUID[0]);

            //Truncate to get relative path
            PACKAGE_ROOT_FOLDER = scriptFilePath.Replace("Scripts/Editor/StylizedWaterCore.cs", string.Empty);
